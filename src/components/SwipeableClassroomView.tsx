...
import LikedClassroomsSection from './LikedClassroomsSection';
...
const SwipeableClassroomView: React.FC<SwipeableClassroomViewProps> = ({ classrooms }) => {
  ...
  // Prepare liked classrooms data for the new component
  const likedClassroomsData = likedClassrooms.map(classroom => ({
    id: classroom.id,
    classroom,
  ...
  const handleEnrollmentComplete = () => {
    // Refresh data or handle state updates
    toast({
  ...
  return (
    <div className="w-full max-w-md mx-auto space-y-4 sm:space-y-6 px-4 sm:px-0">
      ...
      {/* Show enrollment section for liked classes */}
      <LikedClassroomsSection 
        likedClassrooms={likedClassroomsData}
